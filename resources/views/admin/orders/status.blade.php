...
@section('title', 'Update Status Pesanan - ' . $order->kode_pesanan)
...
<div class="p-8">
        ...
        <div class="p-6 bg-gradient-to-r from-orange-50 to-amber-50 border-b border-slate-100">
            <div class="flex flex-wrap items-center gap-6">
                <div>
                    ...
                    <p class="text-xl font-bold text-[#C06044] font-mono">{{ $order->kode_pesanan }}</p>
                </div>
                <div class="w-px h-8 bg-slate-200"></div>
                <div>
                    <span class="text-xs font-medium text-slate-500 uppercase tracking-wider">Customer</span>
                    <p class="text-base font-medium text-slate-800">{{ $order->customer['nama'] ?? '-' }}</p>
                </div>
                <div class="w-px h-8 bg-slate-200"></div>
                <div>
                    <span class="text-xs font-medium text-slate-500 uppercase tracking-wider">Total</span>
                    <p class="text-base font-medium text-slate-800">Rp {{ number_format($order->total, 0, ',', '.') }}</p>
                </div>
                <div class="w-px h-8 bg-slate-200"></div>
                <div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div class="p-6">
            <form method="POST" action="{{ route('admin.orders.update-status', $order->kode_pesanan) }}">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="status_pesanan" required 
                            ...
                            @foreach($statuses as $value => $label)
                                <option value="{{ $value }}" {{ $order->status_pesanan == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <textarea name="catatan_admin" rows="4" 
                                  class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-[#C06044]/20 focus:border-[#C06044] transition-all"
                                  placeholder="Tambahkan catatan internal untuk pesanan ini...">{{ $order->catatan_admin }}</textarea>
                    ...
                    <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                        <a href="{{ route('filament.admin.resources.orders.view', $order->kode_pesanan) }}" 
                ...
                <div class="relative mb-6">
                    ...
                    <div>
                        <p class="text-sm font-medium text-slate-800">
                            Status saat ini: 
                            <span class="status-badge {{ $statusColors[$order->status_pesanan] ?? 'bg-gray-100 text-gray-600' }} ml-2">
                                {{ $statusLabels[$order->status_pesanan] ?? ucfirst($order->status_pesanan) }}
                            </span>
                        </p>
                        <p class="text-xs text-slate-400 mt-1">
                            <i class="far fa-calendar-alt mr-1"></i> {{ $order->updated_at->format('d M Y H:i') }}
                        </p>
                    </div>
                </div>
