...
<body x-data="cartPage()" x-init="loadCart()" class="min-h-screen flex flex-col" style="padding-top: 0;">
  ...
  <main class="flex-grow" style="padding: 40px 20px; max-width: 1200px; margin-left: auto; margin-right: auto; width: 100%;">
    ...
    <div class="animate-fade-in">
      ...
      <div x-show="cart.length === 0" x-cloak class="flex flex-col items-center justify-center py-20 text-[#5C3D2E]">
        ...
        <a href="<?php echo e(route('order.index')); ?>" class="text-[#C6584F] underline font-reenie text-2xl">Mulai Belanja</a>
      </div>
          ...
          <div class="lg:col-span-2 space-y-4">
            <template x-for="(item, index) in cart" :key="item.id">
              <div class="bg-white rounded-2xl p-4 flex items-center gap-6 border border-[#E8DCC4] shadow-sm">
                <img x-bind:src="item.image" x-bind:alt="item.name" class="w-24 h-24 rounded-xl object-cover">
                <div class="flex-grow">
  ...
  <script>
    ...
    function syncCartToSession(cart) {
      return fetch('<?php echo e(route('order.sync-cart')); ?>', {
        method: 'POST',
        headers: {
          ...
          loginBtn.addEventListener('click', function(e) {
            ...
            if (cart.length > 0) {
              ...
              syncCartToSession(cart)
                .then(() => {
                  // Simpan redirect URL
                  localStorage.setItem('redirect_after_login', '<?php echo e(route('order.payment')); ?>');
    ...
    window.debugCart = function() {
      ...
      console.log('Payment route:', '<?php echo e(route('order.payment')); ?>');
      console.log('Login route:', '<?php echo e(route('login')); ?>');
      console.log('CSRF Token:', document.querySelector('meta[name="csrf-token"]')?.content);
      
      // Test route payment
      fetch('<?php echo e(route('order.payment')); ?>', { 
        method: 'HEAD',
...
</html><?php /**PATH C:\Users\keand\OneDrive\Desktop\Sekolah\Kelas 12\pempekbunda75\resources\views/order/cart.blade.php ENDPATH**/ ?>